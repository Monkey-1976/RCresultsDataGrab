...
class Program
{
    ...
    private static void OutputByBest10(RaceMeeting meeting)
    {
        var file = new WriteFile(_filePath + meeting.Name.Replace(" ", "") + "_Best10_" + meeting.Date.ToShortDateString().Replace("/", ""));

        int position = 1;
        foreach (OverallResult result in meeting.OverallResults.OrderBy(o => o.Best10))
        {
            if (result.Best10 > 0)
            {
                _logger.LogInformation($"Position {position.ToString("00")} | {result.Driver},{result.BestLap},{result.Best10},{result.BestLaps.Laps},{result.BestLaps.TotalTime}");
                file.Write($"{result.Driver}: {result.Best10}");
                position++;
            }
        }
    }
